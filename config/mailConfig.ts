...
const sendEmail = async (to: string, subject: string, body: string) => {
  await transporter.sendMail({
    from: `"Your Virtual Remote Book Keeper" <${process.env.EMAIL_USER}>`,
...
export const sendVerificationToEmail = async (to: string, token: string) => {
  ...
  const html = `<h1>Welcom to Virtual Remote Book Keeper </h1>
...
export const sendRestPasswordToEmail = async (to: string, token: string) => {
  ...
  const html = `<h1>Welcom to Virtual Remote Book Keeper rest your password</h1>
     <p> ypu have requst to rest password. Please click on varification link </p><br> <a href="${verificationUrl}"> verification Email</a>`;
  await sendEmail(to, "Please reset password Virtual Remote Book Keeper", html);
}